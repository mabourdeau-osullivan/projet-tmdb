...
const ThrillerMovies = () => {
    // Backend
    const [action, setThriller] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            for (let page = 2; page <= 10; page++) {
                const URL = `https://api.themoviedb.org/3/discover/movie?api_key=8434a0b660ab72dea068e0d5edd42503&with_genres=53&page=${page}`;
                const res = await axios.get(URL);
                allActionMovies = allActionMovies.concat(res.data.results);
            }
            setThriller(allActionMovies);
        };
        fetchData();
    }, []);
    ...
    return (
        ...
        <div className="actionMovies">
            ...
            <h1 className="h1-caroussels">Thriller</h1>
            <Carousel
                swipeable={true}
                draggable={true}
